-- HealthyBowl Database Schema for Supabase (Updated for Order Tracking)
-- This file contains all the necessary tables for the HealthyBowl website with enhanced tracking
...
-- Orders table (Enhanced for tracking)
CREATE TABLE orders (
    id UUID DEFAULT uuid_generate_v4() PRIMARY KEY,
    ...
    -- Order status (Enhanced tracking)
...
CREATE TABLE order_status_history (
    ...
    notes TEXT,
    created_at TIMESTAMP WITH TIME ZONE DEFAULT NOW(),
    created_by VARCHAR(100) DEFAULT 'system' -- system, admin, customer
);
...
BEGIN
    ...
    LOOP
        ...
        -- Get counter for today by counting existing orders
        SELECT COALESCE(COUNT(*), 0) + 1
        INTO counter
        FROM orders
        WHERE order_number LIKE 'HB-' || date_prefix || '%';
        
        -- Format as HB-YYYYMMDD-XXXX
        order_num := 'HB-' || date_prefix || '-' || LPAD(counter::TEXT, 4, '0');
        ...
        -- If we've tried too many times, add microsecond component
        IF attempt >= max_attempts THEN
            order_num := 'HB-' || date_prefix || '-' || LPAD(counter::TEXT, 4, '0') || '-' || EXTRACT(MICROSECONDS FROM NOW())::INTEGER;
            RETURN order_num;
        END IF;
    END LOOP;
END;
...
BEGIN
    ...
    VALUES (p_order_id, p_status, p_notes, p_created_by);
END;
$$ LANGUAGE plpgsql;
...
-- Allow guest order items (when order has user_id = NULL)
CREATE POLICY "Allow guest order items" ON order_items
    FOR INSERT WITH CHECK (
        EXISTS (
            SELECT 1 FROM orders 
            WHERE orders.id = order_items.order_id 
...
-- Allow system to insert status history (for guest orders and system updates)
CREATE POLICY "Allow system status history" ON order_status_history
    FOR INSERT WITH CHECK (created_by = 'system');

-- Allow admin to insert status history (for admin dashboard)
CREATE POLICY "Allow admin status history" ON order_status_history
    FOR INSERT WITH CHECK (created_by = 'admin');

-- Contact messages are public for insertion
CREATE POLICY "Anyone can insert contact messages" ON contact_messages
    FOR INSERT WITH CHECK (true);

-- Order tracking is public (by order number)
CREATE POLICY "Anyone can view orders by order number" ON orders
    FOR SELECT USING (true);

CREATE POLICY "Anyone can view order items by order number" ON order_items
    FOR SELECT USING (true);

-- Allow updates to orders by order number (for admin dashboard)
CREATE POLICY "Allow order updates by order number" ON orders
...
BEGIN
    ...
    INSERT INTO order_status_history (order_id, status, notes, created_by)
    VALUES (v_order_id, 'pending', 'Order placed', 'system');
...
BEGIN
    ...
    -- Add to status history
    INSERT INTO order_status_history (order_id, status, notes, created_by)
    VALUES (v_order_id, p_status, p_notes, p_created_by);
